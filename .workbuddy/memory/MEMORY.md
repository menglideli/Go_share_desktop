...
23. **后台进程启动有 30~90s 延迟**（`run_in_background` 返回 ≠ 进程在跑）：e2e 时序
    不能靠 `-exit` 对齐两个进程的生命窗口（e2e53/54 两轮因此作废）。确定性做法 =
    长寿命 host + 手动按 PID 杀（先按命令行认 PID）。
24. **固定布局面板会超高溢出**：780 高窗口 + 2 个观众，「停止分享」就被推出窗外 ——
    信息区 `layout.List` 滚动、操作区固定底部。加 UI 元素时想一遍面板总高。
25. **重连/换源时旧对象要"一进新流程就关"**：旧 peer 的 `OnState(closed)` 会覆盖
    新会话的状态变量，误杀健康会话。跨屏换源用 `atomic.Pointer`（WaitFrame 会阻塞，
    持锁换源会卡死）。
26. **Gio 0.10 没有 `widget.NoFit`，原名 `widget.Unscaled`**（1:1 原始像素用它 +
    `Scale=1/PxPerDp`）。
